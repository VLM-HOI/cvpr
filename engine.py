...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, (samples, targets, raw_img) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        #print(targets)
        loss_dict = criterion(outputs, targets, raw_img)
...
def evaluate_hoi(dataset_file, model, postprocessors, data_loader, subject_category_id, device, args):
    ...
    for samples, targets, raw_img in metric_logger.log_every(data_loader, 100, header):
        samples = samples.to(device)
        outputs = model(samples)
